...
const ResultsTabs: React.FC<ResultsTabsProps> = ({
  ...
  const renderLoanTypeSelector = () => <div className="mb-4 sm:mb-6">
      <div className="flex items-center gap-1 mb-1">
        <Label htmlFor="resultLoanType" className="text-sm sm:text-base font-medium">Cách tính lãi</Label>
        <Tooltip>
  ...
  const renderMobileStats = () => <div className="flex flex-wrap gap-2 mb-4">
      <MobileStatsCard icon={Building2Icon} title="Tổng Thanh Toán" value={formatCurrency(loanResults.totalPayment)} />
      <MobileStatsCard icon={BarChart3Icon} title="Tổng Lãi" value={formatCurrency(loanResults.totalInterest)} />
    </div>;
  const renderDesktopStats = () => <div className="grid grid-cols-3 gap-6 mb-8">
      <StatsCard icon={Building2Icon} title="Tổng Thanh Toán" value={formatCurrency(loanResults.totalPayment)} />
      <StatsCard icon={BarChart3Icon} title="Tổng Lãi" value={formatCurrency(loanResults.totalInterest)} />
      <StatsCard icon={WalletIcon} title="Khoản Vay Gốc" value={formatCurrency(loanResults.totalPayment - loanResults.totalInterest)} />
    </div>;
  return <div className="card-gradient rounded-xl sm:rounded-2xl shadow-lg p-3 sm:p-8 animate-scale-in">
